...
/* Disable (0) or enable (1) diagnostic functions performing
 * after the calibration. Disabled by default.
 */
#define CEM102_DIAG_FUNCTIONS              (0)

typedef enum {
    CEM102_OFFSET_CURRENT_WE1 = 1,
...
} CEM102_CURR_MEASURE_TYPE;

/* Following definitions must have the value 1 less than CEM102_DAC_TYPE
 * for WE1, WE2, and RE. Besides being used to identify the request
 * type, it is also used as an offset. Fourth value, WE1_WE2 is unique.
 */
typedef enum
{
    CEM102_LSAD_READ_WE1 = (WE1 - 1),
    CEM102_LSAD_READ_WE2 = (WE2 - 1),
    CEM102_LSAD_READ_RE = (RE - 1),
    CEM102_LSAD_READ_WE1_WE2 = 3,
} CEM102_LSAD_READ_TYPE;

typedef enum {
    REG_ANA_CFG0_OFFSET            = 0,
    REG_ANA_CFG1_OFFSET            = 1,
    REG_ANA_SW_CFG0_OFFSET         = 2,
    REG_ANA_SW_CFG1_OFFSET         = 3,
    REG_ANA_SW_CFG2_OFFSET         = 4,
    REG_ANA_SW_CFG3_OFFSET         = 5,

    REG_DC_DAC_RE_CTRL_OFFSET      = 6,
    REG_DC_DAC_WE2_CTRL_OFFSET     = 7,
    REG_SYSCTRL_IRQ_CFG_OFFSET     = 8,

    REG_CHCFG_CH1_LSAD_OFFSET      = 9,
    REG_CHCFG_CH1_THRESHOLD_OFFSET = 10,
    REG_CHCFG_CH1_BUFFER_OFFSET    = 11,
    REG_CHCFG_CH1_ACCUM_OFFSET     = 12,

    REG_CHCFG_CH2_LSAD_OFFSET      = 13,
    REG_CHCFG_CH2_THRESHOLD_OFFSET = 14,
    REG_CHCFG_CH2_BUFFER_OFFSET    = 15,
    REG_CHCFG_CH2_ACCUM_OFFSET     = 16,

    REG_ANA_CFG2_OFFSET            = 17,
    REG_DC_DAC_WE1_CTRL_OFFSET     = 18,
    REG_CHCFG_LSAD_CLK_OFFSET      = 19,

    MAX_REG_COUNT_FOR_SAVE         = 20,
} CEM102_REG_LIST_OFFSET;

/* The number of entries and the order of registes in REGISTER_DATA_VALID
 * must match with the register address list in REG_ADDR_FOR_SAVE. Otherwise, wrong
 * registers will get saved
 */
typedef enum {
    REG_ANA_CFG0_SAVE            = (1U << REG_ANA_CFG0_OFFSET),
    REG_ANA_CFG1_SAVE            = (1U << REG_ANA_CFG1_OFFSET),
    REG_ANA_CFG2_SAVE            = (1U << REG_ANA_CFG2_OFFSET),

    REG_ANA_SW_CFG0_SAVE         = (1U << REG_ANA_SW_CFG0_OFFSET),
    REG_ANA_SW_CFG1_SAVE         = (1U << REG_ANA_SW_CFG1_OFFSET),
    REG_ANA_SW_CFG2_SAVE         = (1U << REG_ANA_SW_CFG2_OFFSET),
    REG_ANA_SW_CFG3_SAVE         = (1U << REG_ANA_SW_CFG3_OFFSET),

    REG_DC_DAC_RE_CTRL_SAVE      = (1U << REG_DC_DAC_RE_CTRL_OFFSET),
    REG_DC_DAC_WE1_CTRL_SAVE     = (1U << REG_DC_DAC_WE1_CTRL_OFFSET),
    REG_DC_DAC_WE2_CTRL_SAVE     = (1U << REG_DC_DAC_WE2_CTRL_OFFSET),
    REG_SYSCTRL_IRQ_CFG_SAVE     = (1U << REG_SYSCTRL_IRQ_CFG_OFFSET),

    REG_CHCFG_LSAD_CLK_SAVE      = (1U << REG_CHCFG_LSAD_CLK_OFFSET),
    REG_CHCFG_CH1_LSAD_SAVE      = (1U << REG_CHCFG_CH1_LSAD_OFFSET),
    REG_CHCFG_CH1_THRESHOLD_SAVE = (1U << REG_CHCFG_CH1_THRESHOLD_OFFSET),
    REG_CHCFG_CH1_BUFFER_SAVE    = (1U << REG_CHCFG_CH1_BUFFER_OFFSET),
    REG_CHCFG_CH1_ACCUM_SAVE     = (1U << REG_CHCFG_CH1_ACCUM_OFFSET),

    REG_CHCFG_CH2_LSAD_SAVE      = (1U << REG_CHCFG_CH2_LSAD_OFFSET),
    REG_CHCFG_CH2_THRESHOLD_SAVE = (1U << REG_CHCFG_CH2_THRESHOLD_OFFSET),
    REG_CHCFG_CH2_BUFFER_SAVE    = (1U << REG_CHCFG_CH2_BUFFER_OFFSET),
    REG_CHCFG_CH2_ACCUM_SAVE     = (1U << REG_CHCFG_CH2_ACCUM_OFFSET),

} CEM102_REG_LIST_SAVE_VALID;

#define REG_ADDR_LIST_FOR_SAVE { \
                                 ANA_CFG0, \
                                 ANA_CFG1, \
                                 ANA_SW_CFG0, \
                                 ANA_SW_CFG1, \
...
#define CEM102_PRE_MEASURE_CONFIG          (1)
#define CEM102_POST_MEASURE_CONFIG         (2)

/* Storage needed for registers modified by BufferConfig, SwitchConfig,
 * ADCConfig APIs
 */
typedef struct {
    uint32_t valid; /* Bit field to mark the valid registers in the data. */
    uint32_t saved; /* Bit field to select the registers that are saved. */
    uint16_t value[MAX_REG_COUNT_FOR_SAVE];
} CEM102_RegSaveData;

struct _DRIVER_CEM102_t;
typedef struct _CEM102_CurrentData {
    bool measure;
    float  div_factor;
    CEM102_LSAD_READ_TYPE lsad_req;
    int (*setup_callback)(const struct _DRIVER_CEM102_t *p_cem102,
                          struct _CEM102_CurrentData *p_data);
    void (*result_callback)(const struct _DRIVER_CEM102_t *p_cem102,
                            struct _CEM102_CurrentData *p_data);
    uint32_t valid_regs;
    uint16_t target_mv;
    float  current_we;
...
/* Definitions for persistent storage support for calibration settings */
typedef struct {
    uint32_t dac_setting[3];
    float gain[2];
    uint16_t vcc_trim;
    uint16_t valid_flag;
    uint32_t reserved[14];
} CEM102_CalibrationData;

typedef struct {
    uint32_t sig_ver;
    uint32_t erase_count;    /* For debugging purposes */
    CEM102_CalibrationData data;
    uint32_t _data_end_;     /* Kept as 0. Not for application layer. */
} CEM102_PersistentMemory;
...
typedef struct _DRIVER_CEM102_t {
    ...
    int     (*RegisterWrite)            (const CEM102_Device *p_cfg, uint8_t addr, uint16_t data);                ///< Pointer to \ref CEM102_Register_Write : Write a register. Blocks with timeout waiting for write to complete.
    ...
    int     (*RegisterRead)             (const CEM102_Device *p_cfg, uint8_t addr, uint16_t *p_data);             ///< Pointer to \ref CEM102_Register_Read : Read a register, Blocks with timeout, waiting for read to complete.
    ...
    void    (*GetLSADConfig)            (const CEM102_Device *p_cfg, uint16_t *p_lsad_cfg, uint16_t *p_mask);     ///< Pointer to \ref CEM102_Get_LSAD_Config : Return LSAD configuration for a given orsd, sys_chop settings.
    ...
    int     (*CalculateADCGain)         (uint32_t channel, uint32_t num_accum);                                   ///< Pointer to \ref CEM102_Signal_CalculateADCGain : Calculate the gain of the ADC.
    int     (*CalculateCurrent)         (uint32_t channel, int32_t lsad_value, float *p_current);                 ///< Pointer to \ref CEM102_Signal_CalculateCurrent : Calculate the current measured using the channel gain.
    ...
    int     (*GetADCGain)               (uint32_t channel, float *adc_gain_value);                                ///< Pointer to \ref CEM102_Signal_GetADCGain : Read the ADC Gain values.
    ...
    int     (*CalculateDACAmplitude)    (const CEM102_Device *p_cfg,
                                         ...
                                         uint16_t *p_dac);                                                        ///< Pointer to \ref CEM102_DAC_Calibrate : Calculate DAC value using OTP.
    int     (*CalculateDACAmplitudeADC) (const CEM102_Device *p_cfg,
                                         uint32_t dac, uint16_t mv,
                                         uint16_t *p_dac);                                                        ///< Pointer to \ref CEM102_DAC_CalibrateWithADC : Calculate DAC value using ADC.
    int     (*RegisterMultipleRead)     (const CEM102_Device *p_cfg, uint8_t addr, uint16_t *p_data,
                                         unsigned int len);                                                       ///< Pointer to \ref CEM102_Register_Multiple_Read : Read multiple, successive registers. Blocks with timeout, waiting for read to complete.
    int     (*RegisterMultipleWrite)    (const CEM102_Device *p_cfg, uint8_t addr, uint16_t *p_data,
                                         unsigned int len);                                                       ///< Pointer to \ref CEM102_Register_Multiple_Write : Write multiple, successive registers. Blocks with timeout, waiting for write to complete.
    int     (*GetCalibratedCurrent)     (uint32_t *p_ibp, uint32_t *p_ibn);                                       ///< Pointer to \ref CEM102_Signal_GetCalibratedCurrent : Get the calibrated current from OTP.
    int     (*RegisterReadModifyWrite)  (const CEM102_Device *p_cfg, uint8_t addr, uint16_t data, uint16_t mask); ///< Pointer to \ref CEM102_Register_ReadModifyWrite : Does Read-Modify-Writes operation on a register, Blocks with timeout, waiting for the operation to complete.
    int     (*AIOConfigInit)            (const CEM102_Device *p_cfg, uint32_t channel,
                                         CEM102_RegSaveData *p_aiocfg);                                           ///< Pointer to \ref CEM102_Signal_AIOConfigInit : Save the value of registers, before modifying them for analog IO measurement.
    int     (*AIOConfigDeinit)          (const CEM102_Device *p_cfg, uint32_t channel,
                                         CEM102_RegSaveData *p_aiocfg);                                           ///< Pointer to \ref CEM102_Signal_AIOConfigDeinit : Restore the modified registers to its prior state.
    int     (*MeasureCurrent)           (const CEM102_Device *p_cfg, CEM102_CURR_MEASURE_TYPE req,
                                         CEM102_CurrentData *p_data);                                             ///< Pointer to \ref CEM102_Signal_MeasureCurrent : Measures various residual currents and offset current.
    int     (*MeasureVoltage)           (const CEM102_Device *p_cfg, int chan, int32_t *p_voltage1,
                                         int32_t *p_voltage2);                                                    ///< Pointer to \ref CEM102_Signal_MeasureVoltage : Measures voltage using ADC, for the selected channel(s).
    int     (*RegisterSave)             (const CEM102_Device *p_cfg, CEM102_RegSaveData *p_data);                 ///< Pointer to \ref CEM102_Register_Save : Saves the register values the can be used for later restoration.
    int     (*RegisterRestore)          (const CEM102_Device *p_cfg, CEM102_RegSaveData *p_data);                 ///< Pointer to \ref CEM102_Register_Restore : Restores the value of the registers saved earlier.
    int     (*SetupADCToMeasureDAC)     (const CEM102_Device *p_cfg, CEM102_DAC_TYPE dac, uint32_t flag);         ///< Pointer to \ref CEM102_DAC_ADC_Setup : Setup switches and ADC to measure DAC voltage.
    int     (*CalibrationErase)         (void);                                                                   ///< Pointer to \ref CEM102_Calibration_Erase : Erase the calibration settings in persistent storage area.
    int     (*CalibrationSave)          (CEM102_CalibrationData *p_data);                                         ///< Pointer to \ref CEM102_Calibration_Save : Saves the calibration settings in persistent storage area.
    int     (*CalibrationValid)         (void);                                                                   ///< Pointer to \ref CEM102_Calibration_Valid : Checks the validity of the calibration data in persistent storage area.
    bool    (*VCCCalibrationValid)      (void);                                                                   ///< Pointer to \ref CEM102_VCC_Calibration_Valid : Checks the validity of the vcc calibration data in persistent storage area.
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
